...
%% NETWORKED SENSORS EXTENDED CONTROL VERSION
...
mainlineclear = 300; % threshold to initially apply to the upstream units light sensor reading if a sensible threshold has not yet been calculated. 
Transfer_Down_matrix_u = []; % matrix to hold the history of the newly read light sensor. 
blockage = 0; % flag to show if tehre is a blockage at the exit point. 

% section to add networked sensors to the unit 
filename_local_tval = ['Tval_Current',Number_of_Feedlines,'.txt']; % look for the sensor attached to the upstream most feed unit over the transfer point 
filepath_local_tval = [path2databus,filename_local_tval]; % find the file in the simualted data bus 
filename_Platformstatus_main =['Platform_Sensor_Data_Current_',Number_of_Feedlines,'.txt']; % hunt down the platofrm sesnor data atatched to the upstream msot transfer unit 
filepath_Platformstatus_main=[path2databus,filename_Platformstatus_main]; % find the relevant file in the simulates data bus. 
...
% instead of checking the transfer sensor attachd to the upstream unit we
% use the network to study the existing sensor at the transfer point to
% determine if a pallet is waiting and hence we need to buffer. 
    disp('Attempting a Networked Sensor Check for a Blockage- Checking Light Sensor')
    toc
    if exist(filepath_local_tval) ~= 0
        % if the file has been created then we want to extract the data
        % from it. 
            fid=fopen(filepath_local_tval,'r');
            out=textscan(fid,'%s %s');
            fclose(fid);
            if isempty(out{1,1})
                % if the file exists but has not been written, but a past
                % file ahs been read then we can use the old value. 
                disp('Data is currently writing-use old value')    
            else     
                % else we log the data into the matrix assigned to the
                % light sensors such that we can review it - sometimes it
                % is interesting to see how the network delays the data
                % from read to write point graphically by comparing it to
                % the graph generated by the unit the sensor is atatched
                % to. 
                val_entering = str2num(out{1,1}{1,1});
                mainlineclear = str2num(out{1,2}{1,1});
                ...
                % now we make a decision about wheterh the pallet is there
                % by threshlding the sensor data. 
        ...
        % else the file ahs not been created, probably as the unit is still
        % getting ready and tehrefore not fed, else the unit is running in
        % data gathering mode only and so no pallets will be fed from the
        % line at all and hecne we never get blockages. 
        disp('The File has no Been Created Yet')
        pallet = 0; 
    end 
    % next we consult the transfer platform - if theere is no pallet but
    % the platform is still raised then the mainline is still unable to
    % accept another pallet without collision and so we delay the entry of
    % the pallet. 
    if exist(filepath_Platformstatus_main)~= 0 
        % open the file. 
            ...
            if isempty(out{1,1})
                % if the cell is empty then we have no data and w use
                % whatever existed previously. 
                disp('Data is currently writing-use old value')    
            else     
                % else if tehre is some data then we make use of it 
                transfer_input = str2num(out{1,1}{1,1});
            end
    else
            % else the file has not yet been created and hence we knwo
            % there is no blockage - else there is a communications error
            % beyond the scope of the programme to resolve. 
 ...
 % make a decision if the transfer is occuring i.e. whether the platform is up OR if the sensor is blocked   
    if transfer_input == 1 || pallet == 1 % if either the platofrm is up or there is a pallet at the end of the 
        % transfer line then we set the blockage flag high to prevent
        % feeding onto the mainline to allow the higher priority pallet to
        % pass. 
        blockage = 1; 
    else
        % if neither of these things are true then there is no blockage. 
        blockage = 0; 
    end      
