% Write_Main_Startup.m
% Given a value of Main_id to denote which line segment the unit corresponds
...
% Main id is known. This file will generate a script which can be run at
...
filename=['Main_',num2str(Main_id),'.m'];
filepath=[path2mainstartup,filename];
...
% with a name Main_X.log. 
fprintf(handle_temp,['diary([path2eventlog,''Main_'',''',num2str(Main_id),''',''.log''])','\n']);
% this prints a command which sets up a variable in the workspace called
% Main_id with a value corresponding to the main line segment
string = ['Main_id = ',num2str(Main_id),';'];
...
% run 
string = ['disp(''Running Mainline ',num2str(Main_id),''')'];
fprintf(handle_temp,[string,'\n']);
...
fprintf(handle_temp,'Mainline_Setup');
