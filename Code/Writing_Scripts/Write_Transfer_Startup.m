% Write_Transfer_Startup.m
% Given a value of Transfer_id to denote which side line the unit corresponds
...
% transfer id is known. This file will generate a script which can be run at
...
filename=['Transfer_',num2str(Transfer_id),'.m'];
filepath=[path2transferstartup,filename];
...
% with a name Transfer_X.log. 
fprintf(handle_temp,['diary([path2eventlog,''Transfer_'',''',num2str(Transfer_id),''',''.log''])','\n']);
% this prints a command which sets up a variable in the workspace called
% Transfer_id with a value corresponding to the feed line the unit is a part of
string = ['Transfer_id = ',num2str(Transfer_id),';'];
...
% run 
string = ['disp(''Running Transfer ',num2str(Transfer_id),''')'];
fprintf(handle_temp,[string,'\n']);
...
if str2num(out{1,3}{ind,1}) == 0
    fprintf(handle_temp,'Transfer_Datagathering');
else
    fprintf(handle_temp,'Transfer_Setup');
end 
