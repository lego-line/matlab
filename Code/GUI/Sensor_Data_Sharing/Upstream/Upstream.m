...
%% NETWORKED SENSORS 1 CONTROL VERSION
...
Code = out{2}(ind);
upstream = COM_OpenNXTEx('USB', Code{1}); %connect to NXT
...
if strcmp(UpstreamMode,'Feed') == 1
    ...
    while Go==2 && Failure_Flag == 0
        ...
        % update the status
        transferline_clear_upstream; % check the light sensor to see if there is a blockage 
