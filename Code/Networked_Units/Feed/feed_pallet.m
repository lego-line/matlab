...
if a == 1 && status(1) == 0 && motoron.IsRunning == 0 && motoron2.IsRunning == 0 && feeding2 == 0 && trap == 0
 ...
 % 05/10/11: Drive conveyor motor to move pallet to light sensor at end of feed unit.
	feeding2 = 1;				
...
if Failure_Flag ==0;
    motoron = move.ReadFromNXT(load);  		%  05/10/11: Checks again whether conveyor motor is running.
end

if 	feeding2 == 1 
    disp('Checking if Successfuly Moved Pallet')
    
    % use the networked data to check if the pallet has arrived 
    if State_Read(1) == 1 
            disp('The Pallet has Been Successfully Transferred')
            status(1)=1;				% 05/10/11: Update status to show soemthing is at b  
            a=0;% 05/10/11: Update 'a'. 
            if motoron.IsRunning == 0
                disp('Successfully moved pallet to sensor')
                disp('Feed conveyor stopped')
                toc
                feed_pause_time = toc;
                feed_pause = 1;
                feeding2 = 0;
    ...
    if a == 1 && motoron.IsRunning == 0 && fault_flag == 0
                fault_flag = 1; 
                disp('Feed conveyor stopped')
                disp('Failed to get pallet to arrive at the sensor- Pallet Jammed upstream') 
                toc
                Network_Write; 
                time_started_error_correct = toc; 
                move_correct.SendToNXT(load);
    end 
    if fault_flag == 1 && (toc - time_started_error_correct) <= 5 && a == 0
                disp('Fault Corrected - No Error')
                toc 
                fault_flag = 0; 
                disp('The Pallet is back in place')
                disp('Successfully moved pallet to sensor after correction')
                toc
                stop_correct.SendToNXT(load); 
                feed_pause_time = toc;
                feed_pause = 1;
                feeding2 = 0;				% 05/10/11: Reset flag to indicate conveyor operation now finished.   
                Network_Write; 
     elseif (toc - time_started_error_correct) > 5 && a == 1 && fault_flag == 1
                disp('Fault not resolved- Now an Error')
                toc
                Failure_Flag = 1;
                error_type = 'The Pallet Got Jammed at The Feed Station';
                Kill_Line;
     else
                disp('Continuing to await resolution to the fault.')
                toc
     end  
end 
        % in either case note that the belt has stopped. 
%%
