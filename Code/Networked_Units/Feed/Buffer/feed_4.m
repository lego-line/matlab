...
while all(status == [1 1 0]) == 1 && Failure_Flag == 0 && Go == 2
    ...
    Network_Write;fault_matrix=[fault_matrix;toc,fault_flag];
        ...
        if pallet_clear == 1 && eval(['Error_Correcting_Transfer_',num2str(feed_id)]) == 1 %&& front_pause == 0
        move.SendToNXT(load); 
...
while all(status == [1 0 0]) == 1 && Failure_Flag == 0 && Go == 2
    ...
    % transfer 
        if pallet_clear == 1 && front_pause == 0 && eval(['Error_Correcting_Transfer_',num2str(feed_id)]) == 0
