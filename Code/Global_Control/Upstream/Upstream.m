...
previous_state_pallet=0; % state of the pallet in the transfer point at last check. 
time_last_edge = 0; % tme of the last edge of a time waveform seen. 
...
first_run = 1;
Global_Read; % read the global bus for the first time. 
if strcmp(UpstreamMode,'Feed') == 1
    ...
    while Go==2 && Failure_Flag == 0
        ...
        disp('Start of Loop')
        toc
% This section uses the global controlelrs input to determine if there is a
% blockage - it takes the state of the upstream most transfer unit and
% checks if it is transferring.
        Global_Read; % read global instructions 
        if (size(State_Read,2)) ~= 0 % look for the state vector 
            if State_Read(size(State_Read,2)) ==  0
                if previous_state_pallet == 1 % look for a falling edge to see if the blockage has gone 
                    time_last_edge = toc; 
                end 
                if (toc - time_last_edge) > 3 
                    % give a few seconds just to elt the pallet get clear
                    % of the transfer point , then we can reset the
                    % blockage flag after the edge. 
                    blockage = 0;
                end 
                previous_state_pallet=0;
            else 
                % if that section of the global satte is empty we  can
                % proceed.
                previous_state_pallet=1;
                blockage = 1; 
            end
        else 
            % else there is no pallet in the point of transfer and so no
            % blockage. 
            blockage = 0;
        end 
% update the status
