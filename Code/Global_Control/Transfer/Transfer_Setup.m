...
ind = strmatch(strcat('Line',num2str(Transfer_id)),out{1},'exact');
if length(ind) > 1
     ind=ind(1);
end 
...
pallet_move = 0; % flag to shw when the pallet at entry is moving 
transfer_flag =0; % flag to show when a transfer is in progress
belt_running_flag =0; % flag to show when the belt is moving 
both_belt_move_flag=0; % flag to show if both belts are currently moving or not. 


% setup variables for the variable belt speed feature 
eval(['Belt_Speed_',num2str(Transfer_id),'=0;'])
Previous_Belt_Speed =0; 

% Define distance according to buffer size based on switch variable to determine how far to mveo the pallet  
if num_b == 1 || num_b == 0;
	dist = 5000;
end
...
disp('Beginning Physical Initiations')
toc 
...
disp('Completed Platform Initiations')
toc
...
disp('Finished Initialising Transfer Arm')
toc
