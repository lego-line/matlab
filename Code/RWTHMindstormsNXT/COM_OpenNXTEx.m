...
%% Create NXT iterator
    ...
    if status 
        if ~SuppressErrors
            ...
            disp('No NXT found on USB bus! Make sure the NXT is turned on and device drivers ("Fantom") are properly installed. Rebooting your NXT might help!')
            quit
...
%% Loop through all NXTs
        ...
        if ~gotNXT && (isempty(hIn.NXTMAC) || strcmpi(hIn.NXTMAC, MAC))
            ...
            if stat
                if ~SuppressErrors
                    ...
                    disp('Could not open desired device! Is it already in use? Rebooting your NXT might help!')
                    quit
    ...
    if ~gotNXT
        if ~SuppressErrors
            ...
            disp('No NXT found that matches your parameters (MAC address or NXT''s name)! Please check the constraints. (Rebooting your NXT might help!)')
            quit
...
%% check if NXT present
    if ~foundNXT
        ...
        if ~SuppressErrors
            disp('No NXT found on USB bus! Make sure the NXT is turned on and access rights in /dev/ are properly set. Rebooting your NXT might help!')
...
%% NXT found, open connection
    ...
    if ErrorWhileOpening || isnumeric(DevHandle)
        if ~SuppressErrors
            ...
            disp('Something went wrong while opening the NXT device via USB (is it already open in another handle?). Please try to reboot the NXT or call COM_CloseNXT(''all'')!')
            quit
