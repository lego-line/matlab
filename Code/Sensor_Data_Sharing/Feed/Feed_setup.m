...
ind= strmatch(['Feed',num2str(feed_id)],out{1},'exact');
if length(ind) > 1
     ind=ind(1);
end 
...
disp('Finished Lift Initialisation')
toc 
...
reset.Stop('off',load);
disp('Finished Pusher Initialisation')
toc
disp('Finished Physical Initialisation')
toc 
%% Motor commands
...
disp('Finished Motor comamnds Set Up')
toc 
