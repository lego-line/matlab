...
Network_Read;
if State_Read(1) == 1
    disp('The Pallet is Detected as Having arrived by the transfer')
else
    disp('No Pallet is detected as having arrived by the transfer')
end 
...
if 	feeding2 == 1 
    disp('Checking if Successfully Moved Pallet')
    Network_Read;
    if State_Read(1) == 1
        disp('The Pallet is Detected as Having arrived by the transfer')
        Network_Arrival = 1;
    ...
    if  motoron.IsRunning == 0 && enteringflag == 1 && motoron3.IsRunning == 0 && Network_Arrival == 1% 05/10/11: Checks when have just finished moving pallet with conveyor from 'a' to 'b'.     			    % 05/10/11: Update 'a'. 
        feed_pause_time = toc;
        feed_pause = 1;
    ...
    elseif  Fault_correct == 1 && enteringflag == 0 && motoron3.IsRunning == 1
         ...
         if toc - time_started_error_correction > 3
                ...
                fault_flag = 1; 
                Network_Write;
                error_type2 = ('ERROR: Pallet has Got Jammed At The Arrival Point and Has Not Arrived at the sensor');
                disp(error_type2)
                warndlg_handle = warndlg('Please Unjam the Pallet at Feed Enrty and Replace Under Sensor',['Feed Unit ',num2str(feed_id),' Warning Dialogue']);
                %waitfor(warndlg_handle)
                pause(5)
                delete(warndlg_handle);
                fault_correct = 0; 
                fault_flag = 0;
                Network_Write;
                status(1)=1;				% 05/10/11: Update status to show soemthing is at b  
